...
def utf8_to_sjis(path_in, path_out):
    ...
    with open(path_in, mode='r', encoding='utf-8') as f:
        s = f.read()
    with open(path_out, mode='w', encoding='sjis') as f:
        f.write(s)
...
def main():
    ...
    # 出力用フォルダを作成
    os.makedirs(f'{path_inputdir}/sjis', exist_ok=True)
    for path_in in l:
        print(f'  {path_in}')
        path_out = os.path.dirname(path_in) + '/sjis/' + os.path.basename(path_in)
        utf8_to_sjis(path_in, path_out)
        print(f'  {path_out}')

if __name__ == '__main__':
    print('_____ξ・ヮ・) < utf8_to_sjis v0.0.1 ________')
